<div>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold tracking-tight">Talent</h1>
    ...
    <x-judge.grid>
        @foreach ($records as $index => $record)
            @php $total = (float) $record->mastery + (float) $record->uniqueness + (float) $record->stage_presence + (float) $record->audience_impact; @endphp
            <x-judge.candidate-card division="ms" :record="$record" wire:key="score-{{ $record->id }}">
                <x-judge.score-field :index="$index" field="mastery" label="Mastery" :max="40" :record="$record" />
                <x-judge.score-field :index="$index" field="uniqueness" label="Uniqueness" :max="30" :record="$record" />
                ...
                <x-slot:footer>
                    <x-judge.totals :total="$total" label="Talent equivalent" :weight="50" />
                </x-slot:footer>
