<div>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold tracking-tight">Formal wear</h1>
    ...
    <x-judge.grid>
        @foreach ($records as $index => $record)
            @php $total = (float) $record->beauty + (float) $record->stage_presence + (float) $record->design + (float) $record->overall_impact; @endphp
            <x-judge.candidate-card division="mr" :record="$record" wire:key="score-{{ $record->id }}">
                <x-judge.score-field :index="$index" field="beauty" label="Beauty & poise" :max="40" :record="$record" />
                <x-judge.score-field :index="$index" field="stage_presence" label="Stage deportment/presence" :max="30" :record="$record" />
                <x-judge.score-field :index="$index" field="design" label="Design & fitting" :max="20" :record="$record" />
                <x-judge.score-field :index="$index" field="overall_impact" label="Overall impact" :max="10" :record="$record" />
                <x-slot:footer>
                    <x-judge.totals :total="$total" label="Formal wear equivalent" :weight="20" />
                </x-slot:footer>
