<div>
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold tracking-tight">Departmental uniform</h1>
    ...
    <x-judge.grid>
        @foreach ($records as $index => $record)
            @php $total = (float) $record->presentation + (float) $record->figure + (float) $record->beauty_poise + (float) $record->overall_impact; @endphp
            <x-judge.candidate-card division="mr" :record="$record" wire:key="score-{{ $record->id }}">
                <x-judge.score-field :index="$index" field="presentation" label="Presentation & neatness" :max="40" :record="$record" />
                <x-judge.score-field :index="$index" field="figure" label="Figure" :max="30" :record="$record" />
                <x-judge.score-field :index="$index" field="beauty_poise" label="Beauty & poise" :max="20" :record="$record" />
                <x-judge.score-field :index="$index" field="overall_impact" label="Overall impact" :max="10" :record="$record" />
                <x-slot:footer>
                    <x-judge.totals :total="$total" label="Departmental uniform equivalent" :weight="20" />
                </x-slot:footer>
