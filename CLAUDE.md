...
- Report pages use `<x-report-header>`: it builds the Overall/Judge 1–3 tabs from the `{route}_judge{n}` naming and the rank/print buttons. Buttons with `data-action-url` (rank, mark top 5, create final sheets) are handled in `app.js`, which does a GET and reloads, or shows an inline error.
