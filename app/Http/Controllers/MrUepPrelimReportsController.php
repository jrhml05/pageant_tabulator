...
class MrUepPrelimReportsController extends Controller
{
    ...
    public function mr_prelim_rank()
    {
        ...
        $get_rank = Mr_ranking::select(DB::raw('SUM(prelim) as total'), 'candidate_id')
    ...
    public function mr_casual_wear_rank()
    {

        for ($x = 2; $x <= 6; $x++) {
            $score = Mr_casualwear_score::where('judge_id', $x)
        ...
        $get_rank = Mr_ranking::select(DB::raw('SUM(casual_wear) as total'), 'candidate_id')
            ->groupBy('candidate_id')
            ->orderBy('total', 'asc')
            ->get();
